    ...
    if mode == "📝 快速記錄":
        ...
        with col1:
            phase_option = st.selectbox(
                "階段",
                ["Phase1", "Phase2", "Phase3", "Phase4", "Phase5", "其他", "自訂"]
            )
            if phase_option == "自訂":
                phase = st.text_input("自訂階段", placeholder="例如：Phase X")
            else:
                phase = phase_option

            module_option = st.selectbox(
                "模組",
                ["記憶模組", "反思模組", "行為調節模組", "知識庫", "微調模組", "情感檢測", "提示詞引擎", "通用", "自訂"]
            )
            if module_option == "自訂":
                module = st.text_input("自訂模組", placeholder="例如：AI評估模組")
            else:
